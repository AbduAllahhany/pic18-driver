...
# 1 "ECU_layer/LCD/../../MCAL_layer/GPIO/../compilier.h" 1
# 10 "ECU_layer/LCD/../../MCAL_layer/GPIO/../compilier.h"
...
# 10 "ECU_layer/LCD/../../MCAL_layer/GPIO/../compilier.h" 2
...
# 35 "ECU_layer/LCD/../../MCAL_layer/GPIO/../mcal_std_types.h"
...
# 2 "ECU_layer/LCD/ecu_LCD.c" 2


static Std_ReturnType lcd_8bit_enable_signal(const _8bit_lcd_config *lcd);
static Std_ReturnType lcd_8bit_set_cursor(const _8bit_lcd_config *lcd, uint8 row, uint8 coulmn);
...
Std_ReturnType ecu_8bit_lcd_initialize(const _8bit_lcd_config * lcd)
{
    ...
    {
        Std_ReturnType l_data_pins_counter = 0x0;
        ret = gpio_pin_intialize(&(lcd->RS));
        ret = gpio_pin_intialize(&(lcd->EN));
        for(l_data_pins_counter=0x0; l_data_pins_counter<8; l_data_pins_counter++)
        {
            ret = gpio_pin_intialize(&(lcd->DATA[l_data_pins_counter]));
        }
        _delay((unsigned long)((20)*(4000000UL/4000.0)));
        ret = ecu_8bit_lcd_send_command(lcd,0x38);
        _delay((unsigned long)((5)*(4000000UL/4000.0)));
        ret = ecu_8bit_lcd_send_command(lcd,0x38);
        _delay((unsigned long)((150)*(4000000UL/4000000.0)));
        ret = ecu_8bit_lcd_send_command(lcd,0x38);

        ret = ecu_8bit_lcd_send_command(lcd,0x01);
        ret = ecu_8bit_lcd_send_command(lcd,0x02);
        ret = ecu_8bit_lcd_send_command(lcd,0x0C);
        ret = ecu_8bit_lcd_send_command(lcd,0x06);
        ret = ecu_8bit_lcd_send_command(lcd,0x38);
        ret = ecu_8bit_lcd_send_command(lcd,0x80);
    }
    return ret;
}
...
Std_ReturnType ecu_8bit_lcd_send_command(const _8bit_lcd_config * lcd, uint8 command)
{
    ...
    {
        uint8 counter = 0x0;
        ret = gpio_write_pin(&(lcd->RS),LOW);
        for(counter = 0x0;counter < 8 ;counter++)
        {
           ret = gpio_write_pin(&(lcd->DATA[counter]),((command >> counter) & (uint8)0x01));
        }
...
Std_ReturnType ecu_8bit_lcd_send_char(const _8bit_lcd_config * lcd, uint8 ch)
{
    ...
    {
        uint8 counter = 0x0;
        ret = gpio_write_pin(&(lcd->RS),HIGH);
        for(counter = 0x0;counter < 8 ;counter++)
        {
           ret = gpio_write_pin(&(lcd->DATA[counter]),((ch >> counter) & (uint8)0x01));
        }
        ret = lcd_8bit_enable_signal(lcd);
    }
    return ret;
        ...
        uint8 row, uint8 colum, uint8 ch)
{
    ...
    else
    {
        ...
        ret = ecu_8bit_lcd_send_char(lcd,ch);
    }
    return ret;
}
Std_ReturnType ecu_8bit_lcd_send_string(const _8bit_lcd_config *lcd, uint8 *str)
{
    ...
    else
    {
         while(*str)
         {
             ret = ecu_8bit_lcd_send_char(lcd,*(str++));
         }
    }
    return ret;
}
        ...
        uint8 row, uint8 colum, uint8* str)
{
    ...
    {
        ret = lcd_8bit_set_cursor(lcd,row,colum);
        ret = ecu_8bit_lcd_send_string(lcd,str);
        ...
        uint8 custom_char[], uint8 mem_pos)
{
    ...
    {
        uint8 count = 0;
        ret = ecu_8bit_lcd_send_command(lcd,0x40 +(mem_pos*8));
        for(count = 0; count < 8; count++ )
        {
            ret = ecu_8bit_lcd_send_char(lcd,custom_char[count]);
        }
        ret = ecu_8bit_lcd_send_char_pos(lcd,row,colum,mem_pos);
    }
    return ret;
}
...
static Std_ReturnType lcd_8bit_enable_signal(const _8bit_lcd_config *lcd)
{
    ...
    {
        ret = gpio_write_pin(&(lcd->EN),HIGH);
        _delay((unsigned long)((5)*(4000000UL/4000000.0)));
        ret = gpio_write_pin(&(lcd->EN),LOW);
    }
    return ret;
}
...
static Std_ReturnType lcd_8bit_set_cursor(const _8bit_lcd_config *lcd, uint8 row, uint8 coulmn){
    ...
    coulmn--;
    switch(row){
        case 0x01 : ret = ecu_8bit_lcd_send_command(lcd, (0x80 + coulmn)); break;
        case 0x02 : ret = ecu_8bit_lcd_send_command(lcd, (0xc0 + coulmn)); break;
        case 0x03 : ret = ecu_8bit_lcd_send_command(lcd, (0x94 + coulmn)); break;
        case 0x04 : ret = ecu_8bit_lcd_send_command(lcd, (0xd4 + coulmn)); break;
        default : ;
    }
    return ret;
}
