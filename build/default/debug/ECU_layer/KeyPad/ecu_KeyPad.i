...
# 1 "ECU_layer/KeyPad/../../MCAL_layer/GPIO/../compilier.h" 1
# 10 "ECU_layer/KeyPad/../../MCAL_layer/GPIO/../compilier.h"
...
# 10 "ECU_layer/KeyPad/../../MCAL_layer/GPIO/../compilier.h" 2
...
Std_ReturnType ecu_keyPad_intialize(keyPad_config *keyPad)
{
    ...
    {
        uint8 row = 0x00, colum = 0x00;
        for(row = 0; row < 4 ;row++)
        {
            ret = gpio_pin_intialize(&(keyPad->ROWS_PINS[row]));

        }
        for(colum = 0 ;colum < 4 ;colum++)
        {
            ret = gpio_pin_intialize(&(keyPad->COLUMN_PINS[colum]));
...
Std_ReturnType ecu_keyPad_read_value(keyPad_config *keyPad,uint8 *val)
{
    ...
    else
    {
        ...
        for(row = 0; row < 4 ;row++)
        {
            ...
            {
                ret = gpio_write_pin(&(keyPad->ROWS_PINS[l_row]),LOW);
            }
            ret = gpio_write_pin(&(keyPad->ROWS_PINS[row]),HIGH);
            for(colum = 0 ;colum < 4 ;colum++)
            {
                logic_t pin_val;
                ret = gpio_read_pin(&(keyPad->COLUMN_PINS[colum]),&pin_val);
                if(pin_val == HIGH)
                {
                   *val = keyPad_btn[row][colum];
